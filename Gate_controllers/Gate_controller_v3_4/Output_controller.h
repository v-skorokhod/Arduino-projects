...
class output_controller {
  ...
  int OCP_reverse_timer = 1; // ms OVERCURRENT PROTECTION reverse timer
...
String output_controller::get_feedback () {
  String val = name + " >> pos: " + String(position()) + "% - o_pow: " + String(output_power) +"% - curr: " + String(read_current()) + "A - set_OC: " + String(overcurrent_val) + "A - end_sw: " + String(end_switch_stat());
  return val;
}
...
float output_controller::read_current() {
  ...
  //Serial.println("PIN_" + String(current_pin[0]) + ": " + String(analogRead(current_pin[0])) + " | " + "PIN_" + String(current_pin[1]) + ": " + String(analogRead(current_pin[1])));
  float val = val_0;
...
int output_controller::end_switch_stat(){
  ...
  if (!end_switch_t.stat) end_switch_t.stat = true;
  ...
  //Serial.println(end_switch_t.time_left());
