...
void setup() {
  ...
  Serial.println("Gate_controller_v3_4");
  Serial.println("###########################");
  delay(100);
  if (TEST) {
    ...
    right_gate.init(A5,A6,10,11,A2,"RIGTH"); // current_pin_1, current_pin_2, forward_pin, reverse_pin, end_switch_pin (пины правой створки)
    left_gate.init(A4,A0,3,9,A3,"LEFT"); // current_pin_1, current_pin_2, forward_pin, reverse_pin, end_switch_pin (пины левой створки)
...
void output_driver () {
  ...
  switch (output_driver_mode) {
    ...
    case 2: // close by timer
      ...
      if (button_hold_t.ready() || (left_gate.finished() && right_gate.finished())) {
        output_driver_mode = 0;
        button_hold_t.stat = false;
      } 
