String filenameIDE = "Gate_controller_v4_5"; // ИЗМЕНЯТЬ ПОСЛЕ ИЗМЕНЕНИЯ НАЗВАНИЯ ФАЙЛА !!! ####################################################################
...
void startDesktop () {
  lcd.setCursor(0,1); lcd.print(" Skorokhod&Co 2023");
  ...
  switch (output_driver_mode) {
   ...
   case 1: // open by timer
    ...
    case 2: // close by timer
      ...
      if (button_hold_tm.ready(button_hold_delay) || (left_gate.finished() && right_gate.finished())) {
        output_driver_mode = 0;
        GARAGE_MODE = false;
      } 
      if (left_gate.error_check() || right_gate.error_check()) {
