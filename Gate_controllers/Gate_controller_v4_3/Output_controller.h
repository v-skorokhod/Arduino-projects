...
class output_controller {
  ...
  byte OCP_reverse_power = 250; // 0..255 OVERCURRENT PROTECTION reverse power (мощность отъезжания назад при сработке защиты по току)
  int OCP_reverse_timer = 1500; // ms OVERCURRENT PROTECTION reverse timer
  float OCP_deadzone = 10.0; // % OVERCURRENT PROTECTION deadzone NO REVERSE
  int OCP_DELAY = 1000; //ms (задержка включения защиты по току после включения)
...
String output_controller::get_feedback () {
  String val = name + " >> pos: " + String(position()) + "% - pwr: " + String(output_power) +"% - curr: " + String(read_current()) + "A - set_OC: " + String(overcurrent_val) + "A - end_sw: " + String(end_switch_stat());
  return val;
}

void output_controller::set(int direction_val) {
  OCP_delay_tm.timer(OCP_DELAY);
  if (prev_direction_val != direction_val) {
    set_changes = true;
    OCP_delay_tm.reset();
  }
  if (OCP_delay_tm.ready()) set_changes = false;
