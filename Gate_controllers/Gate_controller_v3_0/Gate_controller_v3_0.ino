...
int button_pin[4] = {6,4,5,2}; // open, close, wicket, stop (менять пины местами для соответсвия пульта)
...
void setup() {
  ...
  Serial.println("Gate_controller_v3_0");
  Serial.println("###########################");
  delay(100);
  if (TEST) {
    ...
    right_gate.init(A7,A6,10,11,A2,"RIGTH"); // current_pin_1, current_pin_2, forward_pin, reverse_pin, end_switch_pin (пины правой створки)
    left_gate.init(A1,A0,3,9,A3,"LEFT"); // current_pin_1, current_pin_2, forward_pin, reverse_pin, end_switch_pin (пины левой створки)
