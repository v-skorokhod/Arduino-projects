...
class output_controller {
  ...
  float set_overcurrent_val = 3.0; // A (защита по току)
  float max_overcurrent_val = 6.0; // A (макс. ток защиты)
...
void output_controller::init(int val_1, int val_2, int val_3, int val_4, String val_5) {
  current_pin = val_1;
  output_pin = val_2;
  reverse_pin = val_3;
  end_switch_pin = val_4;
  name = val_5;
  pinMode(output_pin, OUTPUT);
  pinMode(reverse_pin, OUTPUT);
  pinMode(end_switch_pin, INPUT);
  EEPROM_cel = output_pin * 10;
...
String output_controller::get_feedback () {
  String val = name + " >> position: " + String(position()) + "% - output_power: " + String(output_power) +"% - current: " + String(read_current()) + "A - end_switch: " + String(end_switch_stat());
  return val;
...
float output_controller::read_current() {
  float val = ((analogRead(current_pin) * coef.ref_voltage / 1023.0) - coef.current_offset) / coef.current; // A
...
void output_controller::reverse(bool val) {
  if (output_power) digitalWrite(reverse_pin, !val); 
  else digitalWrite(reverse_pin, HIGH); 
}
...
bool output_controller::overcurrent_protection() {
  if (read_current() > overcurrent_val && !error_stat) {
    ...
    Serial.println(name + " >> OVERCURRENT PROTECTION!!! " + String(overcurrent_val) + "A");
...
void output_controller::output_check(int input_val) {
  ...
  reverse(reverse_stat);
  if (overcurrent_protection() == false) analogWrite(output_pin, output_val);
}
