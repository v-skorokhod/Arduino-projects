...
String set_time_index = "SET_TIME";
...
void show_terminal_comands () {
  Serial.println();
  Serial.println("***** TERMINAL'S COMMANDS LIST: *****");
  Serial.println("->>> " + debug_serial_index + " - ON/OFF debug Serial");
  Serial.println("->>> " + set_ID_index + "val" + no_resend_index + " - change MODULE_ID (do not save to EEPROM)");
  Serial.println("->>> " + set_time_index + " - SET_TIME_RTC only after soft. update");
  Serial.println();
}

void read_terminal_command () { 
// вкл/выкл данных напрямую с сериал порта *******************************************************
String input_command = Serial_read_data(0);
...
// выставить время часам *******************************************************  
  if (input_command.indexOf(set_time_index) > -1) {
    if (MODULE_ID == 0) {
      Serial.println("SET_TIME_RTC starting... "); 
      set_TIME();
    } else { 
      Serial.println("SET_TIME_RTC not availiable for this controller"); 
    }
  }
// изменение ID *******************************************************  
  if (input_command.indexOf(set_ID_index) > -1) { 
    byte index_length = set_ID_index.length(); 
    String data = input_command.substring(input_command.indexOf(set_ID_index) + index_length, input_command.indexOf(no_resend_index));
    MODULE_ID = atoi(data.c_str());
    MODULE_ID = constrain(MODULE_ID, 0, MAX_ID - 1);
    Serial.println("***** ID changed, (do not saved to EEPROM): "  + String(MODULE_ID) + " *****"); 
    //EEPROM.write(0, MODULE_ID);
    //write_param_EEPROM ();
  }
