const int RPM_FILTER_LENGTH = 30;
...
class output_controller {
  ...
  GMedian<20, float> RPM_median_filter; 
  GMedian<20, float> global_speed_median_filter; 
  Servo myservo;
  my_timer pid;
  my_timer RPM_filter;
  int PID_TIMER = 100; // ms
  int RPM_FILTER_TIMER = 10; // ms
  float PRESSURE_PID_VAL = 40.0; // % per s
  float RPM_PID_VAL = 2.0; // % per s
  float START_POWER_RPMeter = 30.0; // % start in auto RPM mode
  float dp_coef = 0.1; //atm
  float df_coef = 0.1; //l/min
  float dR_coef = 2.0; //RPM
...
float output_controller::RPM (bool filter_stat = false) {
  ...
  if (RPM_sensor_num >= 0) {
    ...
    if(interrupt_period[RPM_sensor_num] > min_RPM_imp_PERIOD) buffer_val = 60.0 / (float(interrupt_period[RPM_sensor_num]) * coef.RPM_holes[RPM_sensor_num] / 1000.0);  // RPM
    ...
    if (RPM_filter.timer_ready()) {
      ...
      output_val = constrain(output_val,0,99);
...
void output_controller::main() { 
  ...
  switch (set_mode) {
    ...
    case 3: // Rev per meter
      ...
      if (global_speed > 0.0) {
        if (global_speed < 19.0) set_RPM = set_RPMeter * global_speed * 16.7; 
...
float output_controller::RPM_controller(float input_RPM) {
  ...
  if (pid.timer_ready()) {
    ...
    out_val = constrain(out_val, -2, 2); //%
    //Serial.println("PID: " + String(out_val) + " d_RPM: " + String(d_RPM) + " input_RPM: " + String(input_RPM) + " set_RPM: " + String(set_RPM));
