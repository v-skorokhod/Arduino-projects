...
#define START_SCREEN_DELAY 1000 //ms
...
int RPM_num_sensor [max_outputs] = {2, -1, -1, -1, -1}; // 0 - D2, 1 - D3, 2 - D18 FOR SEEDER MODULE 
...
float max_RPMeter = 1.5; // rev per meter
float barrel_volume = 200.0; // l
int global_flow_input_num = 2; // 0..2
int herbicide_flow_input_num = 1; // 0..2
...
volatile unsigned long buffer_val_interrupt_period[3];
volatile unsigned long last_interrupt_period[3];
volatile unsigned long interrupt_period_filter[3][INTERRUPT_PERIOD_FILTER_LENGTH];
volatile int filter_pos[3];
...
void calculate_interrupt_period(unsigned long val, byte num) {
  ...
  /*
  interrupt_period_filter[num][filter_pos[num]++] = buffer_val_interrupt_period[num];
  if (filter_pos[num] > INTERRUPT_PERIOD_FILTER_LENGTH) filter_pos[num] = 0;
  interrupt_period[num] = 0;
  for (int i = 0; i < INTERRUPT_PERIOD_FILTER_LENGTH; i++) interrupt_period[num] += interrupt_period_filter[num][i] / INTERRUPT_PERIOD_FILTER_LENGTH;*/
