...
String Switch_mode::get_status () {
  ...
  if (pin_1_stat) {
    ...
    switch (set_mode) {
      ...
      case 3: // RPM
        enable = true;
        set_parameter = map(analog_val, 0.0, 100.0, min_RPMeter * 100, max_RPMeter * 100); // RPmeter
        set_parameter = constrain(set_parameter, min_RPMeter * 100, max_RPMeter * 100); // RPmeter
        set_parameter /= 100.0;
