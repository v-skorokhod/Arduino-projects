...
byte send_pos, send_port_turn, request_pos, request_turn;
...
String fill_control_command (int val_ID, int port_num) {
  String val = start_index + String(val_ID) + ":" + String(port_num) + port_index + control_index;
  String command_val = "-1;0.0;";
  
  for (int i = 0; i < 7; i++) {
    if (switch_config[i][0] == val_ID && switch_config[i][1]) {
...
String inf_request(int val_ID, int port_num) {
  String out_val = start_index + String(val_ID) + ":" + String(port_num) + port_index + get_index + inf_index + end_index;
...
void send_control () {
  ...
  if (send_data.timer_ready()) {
    switch (send_pos) {
      case 0: // 
        send_data_to_RS485(fill_control_command(1,send_port_turn));
        send_port_turn++;
        break;
      case 1: // 
        send_data_to_RS485(fill_control_command(2,send_port_turn));
        send_port_turn++;
