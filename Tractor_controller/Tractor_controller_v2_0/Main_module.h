...
#define NEXTION_UPDATE_PERIOD 100 //ms
#define REQUEST_TIMEOUT 100
#define CHECK_ONLINE_TIME 5000 //ms ожидание устройств в сети после старта программы
...
String device_param (int val_ID) {
  ...
  if (device[val_ID].online) {
    val = String(device[val_ID].voltage,1) + "V*" + String(device[val_ID].current,1) + "A | T_FET:" + String(device[val_ID].MOSFET_temp,0) + "'C";
...
String fill_control_command (int val_ID, int port_num) {
  String val = start_index + String(val_ID) + ":" + String(port_num) + port_index + control_index + toStr(speedT());
