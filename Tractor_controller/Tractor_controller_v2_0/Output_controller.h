...
class output_controller {
  ...
  int RPM_FILTER_TIMER = 100; // ms
  float PRESSURE_PID_VAL = 40.0; // % per s
  float RPM_PID_VAL = 100.0; // % per s
...
float output_controller::pressure () {
  ...
  if (press_pin > 0) {
    output_val = (float(analogRead(press_pin)) * coef.ref_voltage / 1024.0 - press_offset) / press_coef; // V
...
float output_controller::RPM (bool filter_stat = false) {
  ...
  if (RPM_sensor_num > 0) {
    ...
    if(interrupt_period[RPM_sensor_num] > 55.0) buffer_val = 60.0 / (float(interrupt_period[RPM_sensor_num]) * coef.RPM_holes[RPM_sensor_num] / 1000.0);  // RPM
    ...
    if (RPM_filter.timer_ready()) {
      ...
      Serial.println("RPM: " + String(interrupt_period[RPM_sensor_num]) + " | " + String(buffer_val) + " | " + String(output_val));
