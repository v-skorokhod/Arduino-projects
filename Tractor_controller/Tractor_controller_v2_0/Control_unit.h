...
float flow (byte num = 0) {
  float output_val = 60.0 / (float(interrupt_period[num]) / 1000000.0) / coef.flow[num];  // l/min
  if (micros() - last_interrupt_period[num] >= 1000000) output_val = 0.0;
  output_val = constrain(output_val,0.0,999.9);
  return output_val;
}

float RPM (byte num = 0) {
  float output_val = 60.0 / (float(interrupt_period[num]) / 1000000.0);  // RPM
  if (micros() - last_interrupt_period[num] >= 1000000) output_val = 0.0;
  output_val = constrain(output_val,0.0,999.9);
  return output_val;
}
